...
public class TestServiceImpl implements TestService {
    ...
    public boolean hasUserAttemptedTest(Long userId, Long testId) {
        return testResultRepository.existsByUser_IdAndTest_Id(userId, testId);
    }
    ...
    public TestResultDTO submitTest(SubmitTestDTO request) {
        // Prevent multiple attempts: check if a result already exists for this user and test
        if (testResultRepository.existsByUser_IdAndTest_Id(request.getUserId(), request.getTestId())) {
            throw new IllegalStateException("You have already attempted this test.");
        }
        ...
        // Set timeSpentSeconds from request
        int timeSpentSeconds = request.getTimeSpentSeconds();
