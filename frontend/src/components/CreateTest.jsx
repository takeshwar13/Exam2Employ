...
const saveTest = async () => {
  if (testData.title && testData.description && questions.length > 0) {
    try {
      const response = await axios.post('http://localhost:8080/api/test/create', {
        ...testData,
        questions: questions
      });

      alert('Test created successfully!');
      console.log('Saved Test:', response.data);
       // Optionally reset form
      setTestData({ title: '', description: '', timeLimit: '', level: 'beginner' });
      setQuestions([]);
    } catch (error) {
      console.error('Error saving test:', error);
      alert('Failed to save test. See console for error.');
    }
  } else {
